...
const LANGUAGE_COLORS = {
  ...
  Python: 'bg-emerald-950/80 text-emerald-300 border-emerald-800/80',
  Go: 'bg-cyan-950/80 text-cyan-300 border-cyan-800/80',
  Rust: 'bg-orange-950/80 text-orange-300 border-orange-800/80',
  Java: 'bg-red-950/80 text-red-300 border-red-800/80',
  C: 'bg-zinc-900 text-zinc-300 border-zinc-700',
  'C++': 'bg-pink-950/80 text-pink-300 border-pink-800/80',
  HTML: 'bg-orange-950/80 text-orange-400 border-orange-800/80',
  CSS: 'bg-indigo-950/80 text-indigo-300 border-indigo-800/80',
};
...
export default function RepoCard({ repo, status = 'idle', onAnalyze }) {
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-800 hover:border-zinc-700 rounded-2xl p-5 flex flex-col justify-between transition-all duration-200 group shadow-lg hover:shadow-emerald-950/10 font-sans">
      <div className="space-y-3">
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-sans font-bold text-base sm:text-lg text-white group-hover:text-emerald-400 transition-colors flex items-center gap-2 truncate">
            {repo.name}
          </h3>
          <a
      ...
      <div className="pt-4 space-y-4">
        <div className="flex items-center justify-between text-xs text-zinc-400 pb-3.5 border-b border-zinc-900 font-mono flex-wrap gap-2">
          <div className="flex items-center gap-3 flex-wrap">
            {repo.language && (
              <span className={`px-2.5 py-0.5 rounded-md border text-xs font-bold font-mono ${langColorClass}`}>
                {repo.language}
              </span>
            )}
        ...
        <div className="flex items-center justify-between font-sans">
          {status === 'idle' && (
            <button
              ...
              className="w-full py-2.5 px-4 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold rounded-xl text-sm flex items-center justify-center gap-2 transition-all shadow-md shadow-emerald-950/30 active:scale-98"
            >
          ...
          {status === 'queued' && (
            <div className="w-full py-2.5 px-4 bg-amber-950/50 border border-amber-900/80 text-amber-400 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2">
          ...
          {status === 'running' && (
            <div className="w-full py-2.5 px-4 bg-emerald-950/50 border border-emerald-900 text-emerald-300 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2">
          ...
          {status === 'completed' && (
            <div className="w-full py-2.5 px-4 bg-emerald-950/80 border border-emerald-800 text-emerald-400 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center gap-2">
