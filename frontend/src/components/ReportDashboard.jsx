  ...
  return (
    <div className="space-y-6 animate-fade-in print:space-y-6 @container">
      ...
      <div className="grid grid-cols-1 @[700px]:grid-cols-3 gap-5 animate-slide-up-2">
        ...
        {/* Radial Score Gauge */}
        <div className="border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-6 sm:p-8 rounded-3xl flex flex-col items-center justify-center text-center shadow-lg dark:shadow-xl transition-colors duration-200">
          <h3 className="text-xs font-bold text-slate-500 dark:text-zinc-400 uppercase tracking-widest mb-4">Overall Profile Score</h3>
          <div className="relative w-36 h-36 sm:w-44 sm:h-44">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              <circle cx="50" cy="50" r="40" className="stroke-slate-200 dark:stroke-zinc-900" strokeWidth="8" fill="transparent" />
              <circle
                cx="50"
                cy="50"
                r="40"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={`text-4xl sm:text-5xl font-black tracking-tight transition-all duration-300 ${
                overall_score >= 80 ? 'text-emerald-600 dark:text-emerald-400' : overall_score >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'
              }`}>{animatedScore}</span>
              <span className="text-xs text-slate-500 dark:text-zinc-500 font-bold uppercase mt-1">/ 100</span>
            </div>
          </div>
          <span className={`mt-5 px-4 py-1.5 text-xs font-extrabold rounded-full border shadow-sm flex items-center gap-1.5 ${
            ...
            {overall_score >= 90 ? (
              <><ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /><span>Excellent Standing</span></>
            ) : overall_score >= 70 ? (
        ...
        <div className="@[700px]:col-span-2 border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-5 @[700px]:p-8 rounded-3xl flex flex-col justify-between shadow-lg dark:shadow-xl space-y-5 transition-colors duration-200">
          <div>
            <div className="flex flex-col @[480px]:flex-row @[480px]:items-center justify-between gap-4 border-b border-slate-200 dark:border-zinc-900 pb-5">
              <div>
                <h3 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight">
                  Audit Summary Dashboard
                </h3>
                <p className="text-sm text-slate-600 dark:text-zinc-400 mt-1">
                  Target Profile: <span className="text-slate-900 dark:text-white font-bold font-mono">@{username}</span>
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 print:hidden w-full @[480px]:w-auto">
                <button
                  ...
                  className="flex-1 @[480px]:flex-initial py-2.5 px-4 bg-slate-100 dark:bg-zinc-900 hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-800 dark:text-zinc-200 text-xs font-bold transition duration-150 border border-slate-200 dark:border-zinc-750 rounded-xl flex items-center justify-center space-x-1.5 shadow-sm"
                  title="Opt-in to publish public README badge"
                >
                  <BadgeCheck className="w-4 h-4 text-slate-600 dark:text-zinc-300" />
                  <span>Make Score Public</span>
                </button>
                <button
                  onClick={handleGenerateAiReadme}
                  className="flex-1 @[480px]:flex-initial py-2.5 px-4 bg-slate-900 dark:bg-white hover:bg-slate-800 dark:hover:bg-zinc-200 text-white dark:text-black text-xs font-extrabold transition duration-150 rounded-xl flex items-center justify-center space-x-1.5 shadow-md active:scale-95"
                ...
                <button
                  ...
                  className="flex-1 @[480px]:flex-initial py-2.5 px-4 bg-slate-100 dark:bg-zinc-900 hover:bg-slate-200 dark:hover:bg-zinc-800 text-xs font-bold text-slate-800 dark:text-white transition duration-150 border border-slate-200 dark:border-zinc-700 rounded-xl flex items-center justify-center space-x-1.5 shadow-sm"
                ...
                <button
                  ...
                  className="flex-1 @[480px]:flex-initial py-2.5 px-4 bg-slate-100 dark:bg-zinc-900 hover:bg-slate-200 dark:hover:bg-zinc-800 text-xs font-bold text-slate-800 dark:text-zinc-200 transition duration-150 border border-slate-200 dark:border-zinc-700 rounded-xl flex items-center justify-center space-x-1.5"
      ...
      {(summary?.architecture_summary || summary?.readme_evaluation) && (
        <div className="border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-6 rounded-2xl shadow-lg dark:shadow-xl space-y-4 font-sans animate-slide-up-3 transition-colors duration-200">
          <div className="flex items-center space-x-3 border-b border-slate-200 dark:border-zinc-900 pb-3">
            <FileText className="w-5 h-5 text-slate-700 dark:text-zinc-300" />
            <h3 className="text-base font-bold text-slate-900 dark:text-white">AI Architecture &amp; Documentation Evaluation</h3>
          </div>
          <div className="grid grid-cols-1 @[560px]:grid-cols-2 gap-4 text-xs font-mono">
            {summary.architecture_summary && (
              <div className="bg-slate-50 dark:bg-black p-4 rounded-xl border border-slate-200 dark:border-zinc-850 space-y-1">
                <span className="text-[10px] text-slate-500 dark:text-zinc-400 font-bold uppercase tracking-wider block">Repository Architecture</span>
                <p className="text-slate-800 dark:text-zinc-200 leading-relaxed font-sans text-xs">{summary.architecture_summary}</p>
              </div>
            )}
            {summary.readme_evaluation && (
              <div className="bg-slate-50 dark:bg-black p-4 rounded-xl border border-slate-200 dark:border-zinc-850 space-y-1">
                <span className="text-[10px] text-slate-500 dark:text-zinc-400 font-bold uppercase tracking-wider block">README &amp; Documentation Analysis</span>
                <p className="text-slate-800 dark:text-zinc-200 leading-relaxed font-sans text-xs">{summary.readme_evaluation}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 @[800px]:grid-cols-2 gap-6 animate-slide-up-3">
        ...
        <div className="border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-7 rounded-2xl shadow-lg dark:shadow-xl space-y-6 hover:border-slate-300 dark:hover:border-zinc-700 transition-all duration-300">
          <div className="flex items-center space-x-3 border-b border-slate-200 dark:border-zinc-900 pb-4">
            <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 flex items-center justify-center shrink-0">
              <Bot className="w-5 h-5 text-slate-700 dark:text-zinc-300" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">AI Prioritized Insights</h3>
              <p className="text-xs sm:text-sm text-slate-600 dark:text-zinc-400">Targeted corrections recommended for profile readiness</p>
            </div>
          </div>
          ...
          {topIssues.length > 0 ? (
            <div className="space-y-4">
              {topIssues.slice(0, 5).map((item, index) => (
                <div
                  key={index}
                  className="bg-slate-50 dark:bg-black border border-slate-200 dark:border-zinc-850 p-4 rounded-xl flex flex-col items-start justify-between gap-3 shadow-sm hover:border-slate-300 dark:hover:border-zinc-700 transition-all duration-200"
                >
                  <div className="space-y-1 w-full">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <span className="w-5 h-5 rounded-full bg-slate-200 dark:bg-zinc-900 flex items-center justify-center text-[10px] font-bold text-slate-800 dark:text-zinc-300 border border-slate-300 dark:border-zinc-750 shrink-0">
                          {index + 1}
                        </span>
                        <h4 className="font-bold text-slate-900 dark:text-white text-xs sm:text-sm">{item.issue}</h4>
                      </div>
                    ...
                    <p className="text-xs text-slate-600 dark:text-zinc-400 pl-7 leading-relaxed font-normal">{item.justification}</p>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <div className="text-center py-10 bg-slate-50 dark:bg-black rounded-xl border border-dashed border-slate-200 dark:border-zinc-850">
              <div className="flex justify-center mb-2">
                <CheckCircle2 className="w-8 h-8 text-emerald-500" />
              </div>
              <p className="text-xs font-semibold text-slate-700 dark:text-zinc-300">No major issues found! Profile has excellent standing.</p>
            </div>
          )}
        </div>

        {/* IDE Copilot Assistant Launcher Card */}
        <div className="border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-7 rounded-2xl shadow-lg dark:shadow-xl flex flex-col justify-between space-y-6 hover:border-slate-300 dark:hover:border-zinc-700 transition-all duration-300">
          <div className="space-y-4">
            <div className="flex items-center space-x-3 border-b border-slate-200 dark:border-zinc-900 pb-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center">
                  ...
                  <span className="ml-2 px-2 py-0.5 text-[9px] bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 rounded border border-emerald-300 dark:border-emerald-800 uppercase font-bold">Llama-3.3-70B</span>
                </h3>
                <p className="text-xs text-slate-600 dark:text-zinc-400">Contextual AI Assistant powered by Hugging Face &amp; Groq</p>
              </div>
            </div>

            <p className="text-xs text-slate-700 dark:text-zinc-300 leading-relaxed font-sans">
              Need step-by-step guidance to purge secrets with <code className="bg-slate-100 dark:bg-black text-emerald-700 dark:text-emerald-400 px-1.5 py-0.5 rounded border border-slate-200 dark:border-zinc-800 font-mono">git-filter-repo</code>, raise @{username}'s health score to 95+, or apply 1-Click security patches?
            </p>

            <div className="grid grid-cols-1 gap-2 pt-1 font-mono text-[11px]">
              <div className="p-2.5 bg-slate-50 dark:bg-black border border-slate-200 dark:border-zinc-850 rounded-xl text-slate-700 dark:text-zinc-300 flex items-center justify-between">
          ...
          <button
            ...
            className="w-full py-3 bg-emerald-500 hover:bg-emerald-400 text-white font-extrabold text-xs rounded-xl shadow-lg transition active:scale-95 flex items-center justify-center space-x-2 font-sans"
          >
