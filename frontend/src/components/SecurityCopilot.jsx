    ...
    <div className="border border-zinc-850 bg-zinc-950 rounded-2xl p-5 shadow-xl space-y-4 font-mono text-xs flex flex-col h-[520px]">
        ...
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h4 className="font-extrabold text-white text-sm flex items-center">
              ...
              <span className="ml-2 px-2 py-0.5 text-[9px] bg-zinc-900 text-zinc-300 rounded border border-zinc-800 uppercase font-bold">Live Context</span>
            </h4>
      ...
      <div className="flex-1 overflow-y-auto space-y-3.5 pr-2 no-scrollbar">
        {messages.length === 0 ? (
          <div className="text-center py-10 space-y-4 text-zinc-500">
            <Sparkles className="w-8 h-8 text-zinc-400 mx-auto animate-pulse" />
            ...
            <div className="pt-2 flex flex-col space-y-2 max-w-md mx-auto">
              {starterPrompts.map((promptText, idx) => (
                ...
                >
                  ...
                  <Sparkles className="w-3 h-3 text-zinc-400 shrink-0 ml-2" />
                </button>
              ))}
            </div>
          </div>
          ...
          messages.map((msg) => (
            ...
            >
              {msg.role === 'assistant' && (
                <div className="w-6 h-6 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-300 shrink-0 mt-0.5">
              ...
              <div
                className={`p-3 rounded-2xl max-w-[85%] leading-relaxed ${
                  msg.role === 'user'
                    ? 'bg-white text-black font-semibold rounded-tr-none text-xs'
      ...
      >
        <input
          ...
          disabled={loading}
          className="flex-1 bg-black border border-zinc-800 rounded-xl px-3.5 py-2.5 text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-500 text-xs font-mono"
        />
        <button
          type="submit"
          disabled={loading || !inputMsg.trim()}
          className="p-2.5 bg-white hover:bg-zinc-200 disabled:opacity-40 text-black font-bold rounded-xl transition shadow-md shrink-0"
        >
